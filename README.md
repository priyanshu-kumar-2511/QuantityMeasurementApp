...
## 🗓 Day 13 – UC19: Microservices Architecture Decomposition with Service Registry
*(Date: 7-April-2026)*

- Decomposed the monolithic Spring Boot application into **6 independent microservices** following Domain-Driven Design and Separation of Concerns principles.
- Introduced **Eureka Server** as the central service registry for service discovery.
- Each microservice runs independently, registers itself with Eureka, and is reachable via its logical service name (no hardcoded URLs).
- Services introduced:
  - `eureka-server` — Service registry (port 8761)
  - `api-gateway` — Single entry point for all clients (port 8080)
  - `auth-service` — Authentication and user identity (port 8081)
  - `measurement-service` — All quantity operations (port 8082)
  - `user-service` — User profile and conversion history (port 8083)
  - `admin-server` — Spring Boot Admin monitoring dashboard (port 9090)

## API Gateway with JWT Validation Filter and Dynamic Routing
- Introduced **Spring Cloud Gateway** as the reactive API gateway for all client traffic.
- Main Flow
  - Client sends all requests to API Gateway at port 8080.
  - `LoggingFilter` logs every incoming request (method, path, timestamp) for observability.
  - Gateway validates the JWT token from the `Authorization` header before forwarding to downstream services.
  - Routes are resolved dynamically via Eureka service names (no hardcoded host/port):
    - `/api/auth/**` → `auth-service`
    - `/api/user/quantities/**` → `measurement-service`
    - `/api/user/**` → `user-service`
  - Invalid or expired tokens are rejected at the gateway before reaching any service.
  - Requests with valid tokens are forwarded with original headers intact.

## Auth Service Microservice — JWT, Google OAuth2, Redis Token Blacklist

- Extracted all authentication logic from the monolith into a standalone **Auth Service**.
- Main Flow
  - **Registration**: User submits `firstName`, `lastName`, `email`, `password`, `mobileNo`. Password BCrypt-hashed (12 rounds). Welcome email sent asynchronously. Auth-service calls User-service via Feign to sync profile.
  - **Login**: Credentials validated → JWT access token (10 days) + refresh token (30 days) issued.
  - **Logout**: Access token stored in **Redis** blacklist. Refresh token deleted from DB. All future requests with that token rejected.
  - **Forgot Password**: 6-digit OTP generated → stored in DB with 15-minute expiry → emailed to user.
  - **Reset Password**: OTP validated → password updated → all active sessions invalidated.
  - **Refresh Token**: Old refresh token rotated (deleted + new one issued) → new access token returned.
  - **Google OAuth2**: User redirected to Google consent screen → backend exchanges auth code for tokens → user auto-created if first login → JWT issued → redirect to frontend with token.
  - All URL contracts preserved exactly from UC18 monolith (zero Angular frontend changes required).

## Measurement Service Microservice with Redis Caching and Feign Integration

- Extracted all quantity measurement operations from the monolith into a standalone **Measurement Service**.
- Main Flow
  - `JwtAuthenticationFilter` validates incoming JWT and populates `SecurityContext` with `userId` and roles.
  - Controller receives and validates input via `@Valid` → delegates to `IQuantityMeasurementService`.
  - Service resolves unit enum (`LengthUnit`, `WeightUnit`, `VolumeUnit`, `TemperatureUnit`) from `measurementType` string using `getUnitEnum()`.
  - **Compare**: Both values converted to base unit via `convertToBaseUnit()` → compared using `Math.abs(val1 - val2) < 0.0001` tolerance.
  - **Convert**: Value converted to base unit → converted from base unit to target unit via `convertFromBaseUnit()`.
  - **Arithmetic (Add, Subtract, Multiply, Divide)**: Centralized via `performArithmetic()` using `DoubleBinaryOperator`. `TemperatureUnit` blocked at `validateOperationSupport()`.
  - Cross-category safety: `measurementType` equality check throws `IllegalArgumentException` if types differ.
  - Result persisted to MySQL via `QuantityMeasurementRepository` — only for authenticated users (anonymous operations not saved).
  - Redis cache key eviction triggered after every write operation using structured keys: `history:operation:{userId}:{op}`, `history:type:{userId}:{type}`.
  - `UserServiceClient` (Feign) with `UserServiceClientFallback` — syncs conversion records to User Service. Falls back gracefully if User Service is unavailable.
  - Admin users can view and delete history across all users. Non-admin users can only access their own records.
- Supported units:
  - **Length** — FEET, INCH, YARD, CENTIMETER
  - **Weight** — KILOGRAM, GRAM, POUND
  - **Volume** — LITRE, MILLILITRE, GALLON
  - **Temperature** — CELSIUS, FAHRENHEIT, KELVIN (equality + conversion only)

## User Service and Admin Server for Profile Sync and Monitoring
- Introduced **User Service** (port 8083) for user profile management and conversion history storage.
- Introduced **Admin Server** (port 9090) using Spring Boot Admin for centralized monitoring of all microservices.
- User Service — Main Flow
  - Auth-service calls `POST /api/user/profiles/sync` via Feign after every registration — idempotent, skips if profile already exists.
  - Measurement-service calls `POST /api/users/{userId}/history` via Feign after each conversion to persist history.
  - Client fetches all history via `GET /api/users/{userId}/history` (ordered by timestamp descending).
  - Client fetches history filtered by type via `GET /api/users/{userId}/history/{type}`.
  - Client fetches total conversion count via `GET /api/users/{userId}/history/count`.
  - User profile details (firstName, lastName, email, mobileNo) managed via `ProfileController`.
- Admin Server — Main Flow
  - All microservices self-register as Spring Boot Admin clients via `admin-server` URL.
  - Dashboard displays health status, memory usage, environment, HTTP traces, log levels for each service.
  - Secured via basic auth (username: `admin`, password: `admin123`).
  - Actuator endpoints of all services aggregated through Eureka.

🔗 *Code Link:*  
[Day 13 – UC21: Microservices](https://github.com/priyanshu-kumar-2511/QuantityMeasurementApp/tree/feature/UC21-Microservices-Architecture)
